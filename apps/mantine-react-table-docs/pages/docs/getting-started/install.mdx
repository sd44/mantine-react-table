...
> This is the install guide for `mantine-react-table` **V2** and `@mantine/` packages **V8**. MRT V2 will not work with Mantine V6.
...
> `mantine-react-table` V2 requires **Mantine V8.1+** packages as dependencies in your project.
>
...
>
> 1.  `@mantine/core` (^v8.1+)
> 2.  `@mantine/hooks` (^v8.1+)
> 3.  `@mantine/dates` (^v8.1+)
> 4.  `@tabler/icons-react` (3.34.0+)
> 5.  `dayjs` (v1.11+)
> 6.  `clsx` (v2+)
> 7.  `react` and `react-dom` (v19)
